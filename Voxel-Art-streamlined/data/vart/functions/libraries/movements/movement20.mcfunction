...
execute anchored eyes positioned ^ ^ ^10 align xyz positioned ~.5 ~-.25 ~.5 as @e[tag=destination.va] if score @s Player.va = tmp Player.va run teleport @s ~ ~ ~
...
execute as @e[tag=position.va] at @s if score @s Player.va = tmp Player.va unless entity @e[tag=destination.va,distance=..3] run teleport @s ^ ^ ^.1 facing entity @e[tag=destination.va,limit=1] eyes
