...
execute as @e[tag=position.va] at @s if score @s Player.va = tmp Player.va unless entity @e[tag=destination,distance=..1] run teleport @s ^ ^ ^.3 facing entity @e[tag=destination.va,limit=1] eyes
